...
import org.bazar.chat.adapter.outbound.websocket.dto.WebSocketReactionChangedEvent;
...
public interface WebSocketMessageMapper {
    ...
    @Mapping(target = "payload", source = "event", qualifiedByName = "mapToReactionChangedPayload")
    WebSocketReactionChangedEvent map(ReactionChangedEvent event);

    @Named("mapToReactionChangedPayload")
    ReactionChangedPayload mapToReactionChangedPayload(ReactionChangedEvent event);
}
